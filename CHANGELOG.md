...
| 内容 | 位置 | 说明 |
|------|------|------|
| §7.4 为什么 SCM 只需一套方程 | LEARNING.md | 张三教育-收入反事实示例 + 结构不变性原理 + 数学形式化 |

涵盖:
- 三步反事实的叠代直觉 (Abduction → Action → Prediction)
- 结构不变性 (Structural Invariance) — 反事实推理的核心前提
- 失效案例分析 (为什么不能把大学生的方程用在高中学历上)
- 三种应对策略: 中介变量显式建模 / 非线性方程 / 敏感性验证
- Pearl SCM 的数学形式化: M_x 只是 M 的「单行修改」, 不是重新拟合
